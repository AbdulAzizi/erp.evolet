  ...
  >
    <v-list dense nav>
      <div v-for="(item,key) in items" :key="'item'+key">
        <v-list-group v-if="item.items" v-model="item.active" :prepend-icon="item.icon" no-action>
          ...
          <v-list-item v-for="(subItem,key) in item.items" :key="'sybItem'+key" :href="subItem.url">
            <v-list-item-action>
      ...
      dynamicLinks: [
        {
          ...
          text: "АСИ",
          url: "/#",
          responsibilities: ["Куратор Портфел ПК стран", "Руководитель ЭН"],
  ...
  methods: {
    ...
    getLinksOf(user) {
      ...
      return this.dynamicLinks.filter(link => {
        ...
        if (link.divisions.includes(user.division.abbreviation) || link.divisions.includes('*'))
