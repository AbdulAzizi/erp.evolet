    ...
    <v-row>
        ...
        <v-col lg="6" md="6" sm="12">
            ...
            <v-card flat class="pt-0">
                <v-simple-table fixed-header dense height="calc(100vh - 180px)">
                    ...
                    <tbody>
                        <tr v-for="(field, index) in processedFields" :key="index">
                            <td v-if="editAllowed">
                                <v-btn
                                    text
                                    fab
        ...
        <v-col lg="6" md="6" sm="12" class="pt-0">
            ...
            <div v-if="polls.length">
                <h4 class="mx-2 mb-2 font-weight-bold grey--text text--darken-2">Опросы продукта</h4>
    ...
    computed: {
        processedFields() {
            ...
            this.product.fields.forEach(field => {
                let foundMatch;
                fields.forEach(e => {
                    if (e.label == field.label) {
                        e.pivot.value = `${field.pivot.value}, ${e.pivot.value}`;
                        foundMatch = true;
                    }
                });
                if (!foundMatch) fields.push(field);
            });
