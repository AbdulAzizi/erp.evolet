...
const getBaseField = field => {
  ...
  baseInput.props = {
    ...
    hint: field.hint,
    filled: true,
    rounded: "rounded" in field ? field.rounded : true
  };
...
const getTextField = field => {
  ...
  baseField.props["rows"] = baseField.props["rows"]
    ? baseField.props["rows"]
    : 3;
...
export default {
  ...
  computed: {
    formField() {
      ...
      const fieldHasErrors = this.errors && this.errors[fieldData.props.name];

      if (fieldHasErrors) {
        fieldData.props["error-messages"] = this.errors[fieldData.props.name];
      }
      return fieldData;
    }
  },
  watch: {
    ...
    field(val) {
      if (this.field.hasOwnProperty("value")) this.fieldVar = this.field.value;
    }
  },
  methods: {
    validate(value) {
