<template>
    <div>
        ...
        <v-container fluid class="pa-0 mt-3 white">
            <template v-for="(user, index) in timesetUsers">
                <v-row no-gutters :key="'timesetUser-' + index">
                    <div>
                        <span
                        ...
                        >
                        <v-divider />
                        <template v-for="(entry, index) in user.entries">
                            <span
                                :key="'entry-fulname-' + index"
                                style="color: #9b9b9b; display: block; min-height: 20px"
                                :class="
                                    'text-caption px-1 ' + getRowColor(entry)
                                "
                ...
                "
                        ...
                        "
                                            :class="
                                                'text-caption px-1 text-truncate ' +
                                                    getRowColor(entry)
                                            "
                                            >{{ entry.comment }}</span
                                        >
                                ...
                                <v-divider
                                    :key="
                                        'timeset-manager-comment-divider-' +
                                            index
                                    "
                                />
                            </template>
                ...
                "
                        ...
                        "
                                ...
                                <v-divider
                                    :key="'timeset-comment-divider-' + index"
                                />
                            </template>
                        </v-col>
                    </v-row>
                </v-row>
                <pie-chart-3D :data="user.totals" :key="index" />
            </template>
        </v-container>
            ...
            axios
                ...
                .then(resp => {
                    ...
                    for (let i = 0; i < this.entries.length; i++) {
                        ...
                        entry.color = this.colors[
                            this.rand(0, this.colors.length - 1)
                        ];

                        this.timesets.forEach(timeset => {
                            if (
                                this.moment(timeset.start_time).format(
                                    "YYYY-MM-DD"
                                ) == entry.date &&
                            ...
                            if (entry.timeToLeave > entry.sign_out) {
                                entry["early"] = this.moment(
                                    this.moment(
                                        entry.timeToLeave,
                                        "HH:mm"
                                    ).diff(this.moment(entry.sign_out, "HH:mm"))
                                ).format("HH:mm");
                            } else {
                                entry["early"] = "00:00";
                            }
                        ...
                        if (entry.sign_in && entry.sign_out) {
                            ...
                            if (
                                entry["present"] >
                                this.moment(entry.hoursPerDay, "H").format(
                                    "HH:mm"
                                )
                            ) {
                        ...
                        // if it is the same User or not the the last one
                        if (
                            this.entries[i + 1] &&
                            ...
                            if (entry["present"] != "") {
                                totalPresent.add(
                                    this.moment.duration(entry["present"])
                                );
                            }
                            if (entry["late"] != "") {
                                totalLate.add(
                                    this.moment.duration(entry["late"])
                                );
                            }
                            if (entry["overtime"] != "") {
                                totalOvertime.add(
                                    this.moment.duration(entry["overtime"])
                                );
                            }
                            if (entry["early"] != "") {
                                totalEarly.add(
                                    this.moment.duration(entry["early"])
                                );
                            }
                            ...
                            tempEntry.date = this.moment(entry.date).format(
                                "YYYY-MM"
                            );
                            tempEntry.sign_in = "";
                            tempEntry.sign_out = "";
                            ...
                            userEntries.push({ ...tempEntry });

                            let user = resp.data.users.filter(usr => {
                                return (usr.id = entry.user_id);
                            })[0];

                            this.timesetUsers.push({
                                ...user,
                                entries: userEntries,
                                totals: [
                                    {
                                    ...
                                    {
                                        text : 'Приостановленые',
                                        value: user.paused
                                    },
                                    {
                                        text : 'В процессе',
                                        value: user.in_proccess
                                    },
                                ]
                            });
                            console.log(this.timesetUsers);
                            userEntries = [];
                            // clear totals
