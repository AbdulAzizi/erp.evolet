      ...
      <v-col cols="9">
        ...
        <v-tabs-items v-model="tab" style="background-color:#f4f5f7;" class="task-main-content">
                    ...
                    <v-list-item-group color="primary">
                      ...
                      >
                        <v-list-item-content v-if="tether.forms.length != 0">
                          <v-list-item-title @click="displayForm(index)">{{ tether.action_text }}</v-list-item-title>
          ...
          <v-tab-item value="attachments">
            <v-col>
              <attachments :files="task.attachments" :taskId="task.id" :edit="edit" />
            </v-col>
          </v-tab-item>
        </v-tabs-items>
      </v-col>
...
export default {
  ...
  data() {
    return {
      ...
      edit: false,
      attachments: [],
      countFiles: this.item.attachments.length
    };
  },
  created() {
    this.synch();
