      ...
      >
        <template v-slot:item="{ item }">
          <tr :class="item.readed == 0 ? 'grey lighten-2' : 'white'" @click="displayTask(item)">
            ...
            <td>
              {{ item.responsibility_description ? item.responsibility_description.text.length >= 30 ?
              item.responsibility_description.text.substring(0,30): item.responsibility_description.text: "Удалено"}}
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                ...
                <span>
                  Просмотрено
                  {{moment(item.read_at).local().format("lll")}}
                </span>
              </v-tooltip>
            </td>
            <td>
