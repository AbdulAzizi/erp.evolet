        ...
        >
            ...
            >
                <template v-slot:item="{ item }">
                    <tr
                        :class="item.readed == 0 ? 'grey lighten-2' : 'white'"
                        @click="displayTask(item)"
                    >
                        <td @click.stop>
                            <v-menu offset-y>
                                ...
                                <v-list dense>
                                    <v-list-item
                                        @click="mark(item)"
                                        v-if="auth.id == item.responsible.id"
                                    >
                                        <v-list-item-title v-if="item.readed"
                                            >Отметить как не
                                            прочитанное</v-list-item-title
                                        >
                                        <v-list-item-title v-if="!item.readed"
                                            >Отметить как
                                            прочитанное</v-list-item-title
                                    ...
                                    <v-list-item
                                        v-if="taskAuthor(item)"
                                        @click="deleteTask(item)"
                                    >
                                        <v-list-item-title
                                            >Удалить задачу</v-list-item-title
                                        >
                                    </v-list-item>
                                </v-list>
                            </v-menu>
                        ...
                        <td
                            @click.stop="
                                filter(item.priority, 'filterByPriority')
                            "
                        >
                        ...
                        <td>
                            <v-tooltip bottom>
                                ...
                                <div style="max-width: 250px" class="pa-0">
                                    {{ item.description }}
                                </div>
                            </v-tooltip>
                        </td>
                        ...
                        <td
                            @click.stop="
                                filter(item.responsible, 'filterByResponsible')
                            "
                        >
                            <avatar
                                size="30"
                        ...
                        <td
                            @click.stop="
                                filter(item.status.id, 'filterByStatus')
                            "
                        >
                            {{ item.status.name }}
                        </td>
